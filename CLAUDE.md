...
```
main.py (entry point, 159 command registrations, 8 background tasks)
    |
...
managers/ (~19 modules: combat, alchemy, spirit_garden, encounter, sect, boss, rift, trade, bounty, etc.)
    |
...
- **`@migration(version=N)` decorator** (`data/migration.py`): register DB migrations. Current version: **v43** (v42/v43 add spirit_farms garden columns + last_harvest_time wild-plot conversion). All migration tasks take `(conn, config_manager)`. Increment `LATEST_DB_VERSION` when adding.
