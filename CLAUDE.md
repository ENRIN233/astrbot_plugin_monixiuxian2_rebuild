...
managers/ (18 modules: combat, sect, boss, rift, trade, consignment, bank, bounty, dual_cultivation, etc.)
    |
data/ (SQLite CRUD: data_manager.py, database_extended.py, migration.py v26)
```
...
- **`@migration(version=N)` decorator** (`data/migration.py`): register DB migrations. Current version: v26. Increment `LATEST_DB_VERSION` when adding.
