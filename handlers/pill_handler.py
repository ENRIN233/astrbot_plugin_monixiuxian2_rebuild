...
class PillHandler:
    ...
    @player_required
    async def handle_use_pill(self, player: Player, event: AstrMessageEvent, pill_name: str = "", quantity: int = 1):
        """处理服用丹药指令
        ...
        if quantity < 1:
            ...
            return
        if quantity > 999999:
            yield event.plain_result("❌ 单次最多服用999999个！")
            return
        ...
        # 使用丹药
        success, message = await self.pill_manager.use_pill(player, pill_name, quantity)
    ...
    async def handle_show_pills(self, player: Player, event: AstrMessageEvent):
        ...
        if active_effects:
            ...
            for effect in active_effects:
                ...
                remaining_seconds = expiry_time - now if expiry_time > 0 else 0
    ...
    async def handle_pill_info(self, player: Player, event: AstrMessageEvent, pill_name: str = ""):
        ...
        if effect_type == "permanent":
            ...
            info_lines.append(f"\n【服用限制】")
            info_lines.append(f"已服用：{used}/2")
    ...
    def _get_effect_description(self, pill_data: dict) -> str:
        ...
        if subtype == "exp":
            ...
            if pill_id == "util_002":
                lines.append("  抵消一次死亡，复活后不损失任何属性")
            else:
                lines.append("  抵消一次死亡，复活后损失15%属性")
