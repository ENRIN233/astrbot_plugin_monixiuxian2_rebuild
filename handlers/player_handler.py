...
class PlayerHandler:
    ...
    async def handle_player_info(self, player: Player, event: AstrMessageEvent):
        ...
        if player.equipped_weapon:
            inst = await self.db.ext.get_weapon_instance(player.equipped_weapon) if self.db.ext else None
            if inst:
                weapon_name = f"{inst['template_name']}·{inst.get('quality', '?')}（锻造）"
            else:
                weapon_name = f"{player.equipped_weapon[:12]}（锻造）"
        armor_name = player.armor if player.armor else "无"
        if player.equipped_armor:
            inst = await self.db.ext.get_weapon_instance(player.equipped_armor) if self.db.ext else None
            if inst:
                armor_name = f"{inst['template_name']}·{inst.get('quality', '?')}（锻造）"
            else:
