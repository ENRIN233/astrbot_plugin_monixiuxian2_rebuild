...
class ShopHandler:
    ...
    async def handle_view_item(self, event: AstrMessageEvent, item_name: str = ""):
        ...
        item = self.shop_manager.find_item_by_name_any(name)
        if not item:
            yield event.plain_result(f"未找到物品【{name}】，请检查名称是否正确。")
            return

        detail_text = self.shop_manager.get_item_details_full(item)
        acquire_hint = self._get_acquire_hint(item.get('type', ''))

        lines = [detail_text, f"获取途径：{acquire_hint}"]
        yield event.plain_result("\n".join(lines))
